...
def aplot(andvil,andviu,andvin,sdf,farm,name):
    ...
    fig.add_trace(go.Scatter(x=sdf.date, y=sdf.NDVI,mode='lines',name='Health Graph',line = dict(color="#000000",width=4)))
    fig.update_layout(title="Health Graph for"+farm+'(Hover over the graph for controls)',showlegend=True,font=dict(family="Courier New, monospace",size=18,color="RebeccaPurple"))
...
for i in range(len(df1.index)):
    ...
    aplot(andvil1,andviu1,andvin1,sdf1,' Custom Region',id)
...
if status == False:
    ...
    OBJECT_TO_SAVE = 'check_health_status_obj_'+str(DT.date.today())+'.txt'
